...
2. cross_learning - cross_learning only (the current default)
3. cga_dynamic - cross_group with dynamic routing (threshold=0.9), no cross_learning
4. combined - cross_learning + cga_dynamic together
"""
...
logger = logging.getLogger("4-Way Comparison")
...
def evaluate(
    ...
    output_path: Path = typer.Option(
        Path("/tmp/4way_final_results.csv"),
    ...
    1. baseline
    2. cross_learning
    3. cga_dynamic (cross_group with dynamic routing)
    4. combined (cross_learning + cga_dynamic)
    """
    ...
    # 4. COMBINED (cross_learning + cga_dynamic)
    logger.info("=" * 50)
    logger.info("CONFIG 4: COMBINED (cross_learning + cga_dynamic)")
    logger.info("=" * 50)
    results = evaluate_config(cga_pipeline, backtest_configs, batch_size, "combined", cross_learning=True)
    all_results.append(results)
    ...
    pivot_mase = pivot_mase[["baseline", "cross_learning", "cga_dynamic", "combined"]]
    ...
    pivot_wql = pivot_wql[["baseline", "cross_learning", "cga_dynamic", "combined"]]
    logger.info("\n" + pivot_wql.to_string())
    ...
    logger.info("=" * 60)
    for model in ["cross_learning", "cga_dynamic", "combined"]:
