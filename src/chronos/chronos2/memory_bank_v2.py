...
class SeriesMemoryBank(nn.Module):
    ...
    def add_memories(
        ...
        if series_ids is None:
            current_max = self.memory_ids[:self.memory_count.item()].max().item() if self.memory_count.item() > 0 else -1
            series_ids = torch.arange(current_max + 1, current_max + 1 + num_series, device=device)
        ...
        if available_space >= num_series:
            self.memory_keys[current_count:current_count + num_series] = keys
            self.memory_values[current_count:current_count + num_series] = values
            self.memory_ids[current_count:current_count + num_series] = series_ids
            self.memory_count += num_series
        else:
            if num_series >= self.max_size:
                self.memory_keys[:] = keys[-self.max_size:]
                self.memory_values[:] = values[-self.max_size:]
                self.memory_ids[:] = series_ids[-self.max_size:]
                ...
                self.memory_keys[-num_series:] = keys
                self.memory_values[-num_series:] = values
                self.memory_ids[-num_series:] = series_ids
                self.memory_count.fill_(self.max_size)
    ...
    def retrieve(
        ...
        # Get active keys, values, and IDs
        active_keys = self.memory_keys[:current_count]    # (M, d_model) - normalized
        active_values = self.memory_values[:current_count]  # (M, d_model) - RAW
        active_ids = self.memory_ids[:current_count]      # (M,)
        
        # Compute similarities using normalized keys: (B, M)
        similarities = torch.matmul(query_keys, active_keys.T)
        
        # ID-based exclusion (much more reliable than similarity-based)
        ...
        similarities = torch.where(valid_mask, similarities, torch.full_like(similarities, float('-inf')))
...
class ParameterFreeMemoryFusion(nn.Module):
    ...
    def forward(
        ...
        if valid_rows.any():
            ...
            # Handle any remaining NaNs (from empty rows that slipped through)
        ...
        # High similarity -> gate close to 1
        # Below threshold -> gate close to 0
        confidence = torch.sigmoid(
            self.temperature * (max_sim - self.threshold)
        )  # (B, 1)
        
        # Compute delta with NaN hardening
        memory_vec = torch.nan_to_num(memory_vec, nan=0.0, posinf=0.0, neginf=0.0)
        delta = memory_vec - query  # (B, d)
        delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0)
        
        # Apply gated fusion
        output = query + confidence * delta
        
        # Update diagnostics (including max_sim stats)
...
class MemoryAugmentedForecasterV2(nn.Module):
    ...
    def _pool_hidden_states(
        ...
        if self.config.pooling_method == "last":
            # Use last token (often most informative)
    ...
    def augment_representations(
        ...
        if series_ids is not None and mask.any():
            # Check if any retrieved ID matches the query ID
            # retrieved_ids: (B, K), series_ids: (B,)
            self_hits = (retrieved_ids == series_ids.unsqueeze(-1)) & mask
            self_hit_rate = self_hits.any(dim=-1).float().mean().item()
        
        # Apply fusion
        augmented_series, diagnostics = self.fusion(
        ...
        if diagnostics['valid_rows'] > 0:
            ...
            self._stats['self_hit_rate'] = 0.9 * self._stats.get('self_hit_rate', 0) + 0.1 * self_hit_rate
